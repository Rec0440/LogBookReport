...
class FlightTime:
    ...
    def collect_single_engine(self, name_sheets, workbook, yr, end='56'):
        ...
        for sheet in name_sheets:
            if sheet.isdigit() and sheet >= end_sheet:  # if title is not report and not later 2019 (reverse counting)
                for row in workbook[sheet].iter_rows(min_row=4):  # next row in worksheet, row[0] - date, row[10] - time
                    ...
                    if isinstance(col_0, datetime):       # try find date type
                        ...
                        if row[7].value and not row[9].value:
                            ...
                            h, m, s = data.split(':')
                            self.main_data[month]['flight_time']['single_engine']['hour'].append(int(h))
                            self.main_data[month]['flight_time']['single_engine']['min'].append(int(m))
        ...
        for month in self.months_digits:  # count total time for month
            self.main_data[month]['flight_time']['total_flight_time']['hour'].extend(
                self.main_data[month]['flight_time']['single_engine']['hour'])
            self.main_data[month]['flight_time']['total_flight_time']['min'].extend(
                self.main_data[month]['flight_time']['single_engine']['min'])
            # below return string type
            self.main_data[month]['flight_time']['single_engine']['sum'] = self.month_sum(month, 'single_engine')
